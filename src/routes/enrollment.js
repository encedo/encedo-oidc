...
router.post('/submit',
  ...
  async (req, res, next) => {
  try {
    const { token, hsm_url, kid, pubkey, signature, genuine } = req.body;
    ...
    // Verify the signature over the challenge using the submitted pubkey.
    // If valid: submitter has access to the private key — cannot be faked
    // with an imported public-only key or a randomly guessed pubkey.
    let sigValid = false;
    try {
    ...
    // ── HSM attestation ───────────────────────────────────────────
    // genuine = device attestation blob from GET {hsm_url}/api/system/config/attestation.
    // TODO: walidacja w attestation.js — POST api.encedo.com (endpoint do ustalenia).
    // Na razie: genuine niepuste → hw_attested=true.
    const attestResult = await validateAttestation(genuine);
    const { hw_attested } = attestResult;
    if (hw_attested !== 'true') {
      console.warn(`[Enrollment] Attestation not verified for sub=${sub}: hw_attested=${hw_attested} reason=${attestResult.reason ?? '-'}`);
    }
    ...
    if (attestResult.eid) userFields.hsm_eid = attestResult.eid;

    await redis.hSet(`user:${sub}`, userFields);
    await redis.hDel(`user:${sub}`, 'enrollment_token');

    await logSecurity(SEC.ENROLL_OK, { sub, username, kid: expectedKid, hw_attested, eid: attestResult.eid ?? null, ip: req.ip });
    console.log(`[Enrollment] Enrolled sub=${sub} (${username}): kid=${expectedKid} hw_attested=${hw_attested}`);
